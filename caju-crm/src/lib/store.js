...
// ---------- Motor de follow-up ----------
// Retorna { alerts: [...], autoMoves: [{leadId, motivo}] }
export function avaliarFollowups(leads, agora = new Date()) {
  ...
  for (const lead of leads) {
    ...
    // 1) Abordagem enviada / Respondeu → follow-up a cada 2 dias, máx 3, depois Perdido
    ...
    // 2) Reunião agendada → lembrete 1 dia antes
    ...
    // 3) Reunião realizada → follow-up manual na data escolhida
    ...
    // 4) Proposta apresentada → a cada 2 dias, máx 3, depois Perdido (não respondeu proposta)
...
function seedLead(base, timeline) {
  // timeline: [[estagio, diasAtras], ...] em ordem cronológica
...
function seedLeads() {
  ...
  const l1 = seedLead(
    ...
    [['abordagem_enviada', 68], ['respondeu', 66], ['lq', 64], ['reuniao_agendada', 62], ['reuniao_realizada', 58], ['proposta', 55], ['cliente', 50]]
  )
  ...
  const l2 = seedLead(
    ...
    [['abordagem_enviada', 34], ['respondeu', 31], ['lq', 28], ['reuniao_agendada', 26], ['reuniao_realizada', 22], ['proposta', 20], ['cliente', 15]]
  )
  ...
  const l3 = seedLead(
    ...
    [['abordagem_enviada', 9], ['respondeu', 7], ['lq', 6], ['reuniao_agendada', 5], ['reuniao_realizada', 3], ['proposta', 2]]
  )
